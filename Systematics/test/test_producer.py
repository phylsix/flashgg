...
process.tightTagDumper = createTagDumper("VHTightTag")
process.tightTagDumper.dumpTrees = True
process.tightTagDumper.dumpWorkspace = False
process.tightTagDumper.quietRooFit = True

process.tightUntaggedDumper = createTagDumper("VHTightTag")
process.tightUntaggedDumper.src = cms.InputTag("flashggVHTightUntagged","untagged")
process.tightUntaggedDumper.dumpTrees = True
process.tightUntaggedDumper.dumpWorkspace = False
process.tightUntaggedDumper.quietRooFit = True

process.looseTagDumper = createTagDumper("VHLooseTag")
process.looseTagDumper.dumpTrees = True
process.looseTagDumper.dumpWorkspace = False
process.looseTagDumper.quietRooFit = True

process.looseUntaggedDumper = createTagDumper("VHLooseTag")
process.looseUntaggedDumper.src = cms.InputTag("flashggVHLooseUntagged","untagged")  
process.looseUntaggedDumper.dumpTrees = True
process.looseUntaggedDumper.dumpWorkspace = False
process.looseUntaggedDumper.quietRooFit = True

process.etTagDumper = createTagDumper("VHEtTag")
process.etTagDumper.dumpTrees = True
process.etTagDumper.dumpWorkspace = False
process.etTagDumper.quietRooFit = True

process.etCutDumper = createTagDumper("VHEtTag")
process.etCutDumper.src = cms.InputTag("flashggVHEtCut","cut")
process.etCutDumper.dumpTrees = True
process.etCutDumper.dumpWorkspace = False
process.etCutDumper.quietRooFit = True

process.etUntaggedDumper = createTagDumper("VHEtTag")
process.etUntaggedDumper.src = cms.InputTag("flashggVHEtUntaggedTag")
process.etUntaggedDumper.dumpTrees = True
process.etUntaggedDumper.dumpWorkspace = False
process.etUntaggedDumper.quietRooFit = True

process.hadTagDumper = createTagDumper("VHHadronicTag")
process.hadTagDumper.dumpTrees = True
process.hadTagDumper.dumpWorkspace = False
process.hadTagDumper.quietRooFit = True

process.hadDiJetDumper = createTagDumper("VHHadronicTag")
process.hadDiJetDumper.src = cms.InputTag("flashggUntaggedVHHadronicTag","untaggedDijet")
process.hadDiJetDumper.dumpTrees = True
process.hadDiJetDumper.dumpWorkspace = False
process.hadDiJetDumper.quietRooFit = True

process.hadOriDiJetDumper = createTagDumper("VHHadronicTag")
process.hadOriDiJetDumper.src = cms.InputTag("flashggVHHadronicTagOriginal","untaggedDijet")
process.hadOriDiJetDumper.dumpTrees = True
process.hadOriDiJetDumper.dumpWorkspace = False
process.hadOriDiJetDumper.quietRooFit = True

process.thadTagDumper = createTagDumper("TTHHadronicTag")
process.thadTagDumper.dumpTrees = True
process.thadTagDumper.dumpWorkspace = False
process.thadTagDumper.quietRooFit = True

process.tlepTagDumper = createTagDumper("TTHLeptonicTag")
process.tlepTagDumper.dumpTrees = True
process.tlepTagDumper.dumpWorkspace = False
process.tlepTagDumper.quietRooFit = True

process.vbfTagDumper = createTagDumper("VBFTag")
process.vbfTagDumper.dumpTrees = True
process.vbfTagDumper.dumpWorkspace = False
process.vbfTagDumper.quietRooFit = True

process.untaggedTagDumper = createTagDumper("UntaggedTag")
process.untaggedTagDumper.dumpTrees = True
process.untaggedTagDumper.dumpWorkspace = False
process.untaggedTagDumper.quietRooFit = True
...
cfgTools.addCategories(process.looseTagDumper,
			[
			("All","1",0)
			],
...
cfgTools.addCategories(process.looseUntaggedDumper,
			[
			("All","1",0)
			],
			variables= dipho_var + vhlVariables,
...
cfgTools.addCategories(process.tightTagDumper,
			[
			("All","1",0)
			],
			variables= dipho_var + vhtVariables,
			histograms=[
			#
			]
		)

cfgTools.addCategories(process.tightUntaggedDumper,
			[
			("All","1",0)
			],
			variables= dipho_var + vhtVariables,
...
cfgTools.addCategories(process.etTagDumper,
			[
			("All","1",0)
			],
			variables= dipho_var + vhetVariables,
			histograms=[
			#
			]
		)

cfgTools.addCategories(process.etCutDumper,
			[
			("All","1",0)
			],
			variables= dipho_var + vhetVariables + vhetCutVariables,
			histograms=[
			#
			]
		)

cfgTools.addCategories(process.etUntaggedDumper,
			[
			("All","1",0)
			],
			variables= dipho_var + vhetVariables,
...
cfgTools.addCategories(process.hadTagDumper,
			[
			("All","1",0)
			],
			variables= dipho_var + vhhVariables,
			histograms=[
			#
			]
		)

cfgTools.addCategories(process.hadOriDiJetDumper,
			[
			("All","1",0)
			],
			variables= dipho_var + vhhVariables,
...
cfgTools.addCategories(process.thadTagDumper,
			[
			("All","1",0)
			],
			variables= dipho_var,
...
cfgTools.addCategories(process.tlepTagDumper,
			[
			("All","1",0)
			],
			variables= dipho_var,
...
cfgTools.addCategories(process.untaggedTagDumper,
			[
			("All","1",0)
			],
			variables= dipho_var,
...
cfgTools.addCategories(process.hadDiJetDumper,
			[
			("All","1",0)
			],
			variables= dipho_var + vhhVariables,
...
process.p = cms.Path(process.dataRequirements*process.customSequence*process.flashggTagMerger*process.flashggVHEtUntaggedTag*process.flashggVHEtCut*process.etCutDumper*process.flashggVHLooseUntagged*process.flashggVHTightUntagged*process.looseUntaggedDumper*process.looseTagDumper*process.tightUntaggedDumper*process.tightTagDumper*process.vbfTagDumper*process.thadTagDumper*process.tlepTagDumper*process.etTagDumper*process.hadTagDumper*process.hadDiJetDumper*process.etUntaggedDumper*process.tagsDumper)
