...
namespace flashgg {
    class VHCSampleProducer : public EDProducer
    {

    public:
        typedef math::XYZPoint Point;

        VHCSampleProducer( const ParameterSet & );
    private:
    ...
    VHCSampleProducer::VHCSampleProducer( const ParameterSet &iConfig ) :
    ...
    void VHCSampleProducer::produce( Event &evt, const EventSetup & )
    {
        ...
        std::auto_ptr<vector<VHLooseTag> > vhloose( new vector<VHLooseTag> );
...
//        }
        ...
        for( unsigned int diphoIndex = 0; diphoIndex < diPhotons->size(); diphoIndex++ ) {
            ...
            VHLooseTag vhloose_obj( dipho, mvares );
            vhloose_obj.includeWeights( *dipho );
...
//------>MET info
            ...
            if( ((lead_pass && !sublead_pass) || (!lead_pass && sublead_pass)) && (hasGoodElec || hasGoodMuons)) {
                ...
                vhloose_obj.setElectrons( tagElectrons );
                vhloose_obj.setMET( tagMETs );
                ...
                if( ! evt.isRealData() ) {
                    ...
                    vhloose->back().setTagTruth( edm::refToPtr( edm::Ref<vector<TagTruthBase> >( rTagTruth, idx++ ) ) );
                }
            }
            if(vhloose.get() == NULL) {cout<<"vhloose null."<<endl;} 
        }
        //if(vhloose.get() == NULL) {cout<<"vhloose null."<<endl;} 
        evt.put( vhloose );
        evt.put( truths );
...
typedef flashgg::VHCSampleProducer FlashggVHCSampleProducer;
